var TAG = "orderCancellationEmail.js";
...
exports.getEmailBody = function getEmailBody(data, skuids, callback){
	...
	var sellerId = data.orderEntity.orderInfo.sellerInfo.sellerId;
    ...
    var sellerfinancials = data.orderEntity.financials.seller;
    var orderfinancials = {};
    ...
    for(var i = 0; i < sellerfinancials.length; i++)
    {
        ...
        { 
        	...
        	{
	         orderfinancials ={"shippingAndHandlingCharges" : numberConversions.get2Decimalpoint(sellerfinancials[i].sellerTotal.shippingAndHandlingCharges)};
	        }
	        else
	        {
	        	orderfinancials ={"shippingAndHandlingCharges" : numberConversions.get2Decimalpoint(sellerfinancials[i].sellerTotal_A.shippingAndHandlingCharges)};
	        }        
        }  
    }
	...
	var reasonForCancellation = null;
    ...
    for(var i = 0; i < orderItems.length; i++){
		...
		// reason for Cancellation remains same for all Line Item.
		reasonForCancellation =	orderItems[i].reasonForCancellation;
	};
	...
	// If reasonForCancellation exists display the reason else keep it as before.
	if(reasonForCancellation === null || reasonForCancellation.toString().trim().length === 0){
		reasonForCancellation = ".";
	}else{
		reasonForCancellation = 'because "'+reasonForCancellation+'".';
	}
				...
				'<tbody>'+
					...
					'<tr style="margin-right:30px;margin-left:31px;float:left;width:87%;">'+
					  '<td style="color:#545454;font-size:14px">'+
						  ...
						  '<p style="color:#000;font-size:14px;line-height:30px;margin:0;">Greeting from mSupply.com</p>'+
						  '<p style="color:#000;font-size:14px;line-height:30px;margin:0;">You have successfully cancelled the following order placed by the customer '+reasonForCancellation+'</p>'+
					  '</td>'+
					'</tr>'+
					'<tr>'+
