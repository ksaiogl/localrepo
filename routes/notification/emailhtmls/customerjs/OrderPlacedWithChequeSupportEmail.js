...
var TAG = "OrderPlacedWithChequeSupportEmail- ";
...
//get all order finance details.
  if(!(emailBodyParams.subTotal === null || emailBodyParams.discount === null ||
        ...
        emailBodyParams.serviceCharges === null || emailBodyParams.grandTotal === null ))
    {
      ...
      var emailSubject = "Online Order [Order No: "+orderId+"] placed via Cheque and is on hold";
              ...
              '<tbody>'+
                ...
                '<tr style="margin-right:30px;margin-left:31px;float:left;width:87%">'+
                  '<td style="color:#545454;font-size:14px">'+
                    '<p style="margin-top:20px;color:#444444;font-size:18px;font-weight:normal;line-height:20px;margin-bottom:10px"><b> Hi mSupply</b>, </p>'+
                    '<p style="color:#000;font-size:12px;line-height:20px">You Have Received the following online order.The same is ON Hold as it was placed via cheque.</p>'+
                    '<p style="color:#000;font-size:12px;line-height:10px">Make Sure you manually confirm it post cheque clearence.</p>'+
                  '</td>'+
                '</tr>'+
                '<tr>'+
              ...
              '<tr>'+
                '<td style="padding:4px 0 0;text-align:center">'+
                  '<p style="margin:0;float:left;width:100%;color:#637279;font-size:10px">'+
                     ...
                     '<a style="text-align:center;text-decoration:none;color:#637279;font-weight:normal" href="mailto:'+htmlGenericvalues.support_email+'" target="_blank">'+htmlGenericvalues.support_email+'</a>'+
  ...
  {
    logger.error(TAG + "OrderFinance details received have null values - OrderPlacedWithChequeSupportEmail- getHtml");
    return callback(true, "Null Validation Error");
  }     
}
catch(e)
{
  console.log(TAG + "Exception in HTML OrderPlacedWithChequeSupportEmail- getHtml - " + e);
  logger.error(TAG + "Exception in HTML OrderPlacedWithChequeSupportEmail- getHtml- :- error :" + e);
